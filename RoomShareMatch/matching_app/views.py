...
def save_roommate_preference(request):
    ...
    if request.method == 'POST':
        ...
        if all([
            ...
            context['cohabitation_form'] = cohabitation_form
            context['layout_form'] = layout_form
            context['rent_form'] = rent_form
            context['roommate_preference_form'] = roommate_preference_form

            return render(request, 'matching_app/roommate_preference.html', context)



    # GETリクエストの場合、現在の設定を表示する
    purpose_form = PurposeForm()
    cohabitation_form = DesiredCohabiteeForm()
    layout_form = RoomLayoutForm()
    rent_form = RentForm()
    roommate_preference = RoommatePreference.objects.get(user=request.user)
    roommate_preference_form = RoommatePreferenceForm(instance=roommate_preference)

    context['purpose_form'] = purpose_form
    context['cohabitation_form']
